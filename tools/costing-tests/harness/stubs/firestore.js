...
const store = { ingredients: new Map(), recipes: new Map(), costing_settings: new Map(), prices: new Map(), products: new Map(), estimates: new Map(), requests: new Map() };
...
export function onSnapshot(ref, cb){ 
  ...
  else if (ref.type==='doc') { const m=store[ref.path[0]]; const d=m&&m.get(ref.path[1]); cb({exists:()=>!!d, data:()=>d}); }
  return ()=>{}; }
...
export async function setDoc(ref, data){ const m=store[ref.path[0]]; if(m){ m.set(ref.path[1], {...(m.get(ref.path[1])||{}), ...data}); notify(ref.path[0]); } window.__writes=(window.__writes||0)+1; }
...
function notify(name){ for (const l of listeners) if (l.ref.path[0]===name) l.cb(snapOf(name)); }
