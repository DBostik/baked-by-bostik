...
const q4 = P.quoteLines({ items: [{ productId: 'cupcakes', qty: 3, tierId: products.get('cupcakes').tiers[0].id }, { productId: 'celebration-cake', qty: 3, tiers: [{ sizeId: '8' }] }] }, ctx, settings);
eq('mixed quantities: line totals add up to the suggested price within a cent', q4.total, q4.estimateTotals.suggested, 0.011);
const q5 = P.quoteLines({ items: [{ productId: 'cupcakes', qty: 3, tierId: products.get('cupcakes').tiers[0].id }, { productId: 'celebration-cake', qty: 1, tiers: [{ sizeId: '8' }] }] }, ctx, settings);
eq('with a single cake in the order the lines add up exactly', q5.total, q5.estimateTotals.suggested, 0.001);
eq('line order is preserved', q5.lines[0].name.startsWith('Custom Cupcakes'), true);
eq('mixed quantities: every line total is price x qty', q4.lines.every(l => Math.abs(l.price * l.qty - l.total) < 0.001), true);
