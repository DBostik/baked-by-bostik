...
- Second review pass: stock explosion reports sub-recipe lines it cannot weigh instead of skipping them; a recipe with no lines is "not complete" rather than $0; inventory start date readable at once; correct before/after when one save touches an item twice; order undo not limited to recent moves; Costing strip placement and late refresh in Request Details; Home margin table marks incomplete costs; honest toasts for backdated prices, partial stock-outs and failed quote pushes; jump threshold means "more than"; scheduled-task prompts fixed for fresh shells.
